...
describe("Encounter Generator", () => {
  describe("peril level", () => {
    list{GroupLevel, GroupLevelMinus1}->List.forEach(level => {
  ...
  describe("experience points for peril", () => {
    allPerilDescriptions->List.forEach(((perilType, level, label, expectedExperience)) => {
      test("experience points for " ++ label, () => {
        expect(experiencePointForPeril(aPeril(perilType, level))) |> toEqual(expectedExperience)
      })
    })
  })
})