...
describe("Encounter Generator", () => {
  ...
  describe("experience points", () => {
    test("experience points of an encounter with 2 simple danger only is 16 points", () => {
      expect(
        experiencePoints(newEncounter->containing(aPeril(SimpleDanger, GroupLevel), 2)),
      ) |> toEqual(16)
    })
    test(
  ...
  describe("difficulties", () => {
    test("as strings", () => {
      expect(Js.Array.joinWith(" ", difficulties->Array.map(difficultyToString))) |> toEqual(
        "Trivial Weak Moderate Serious Extreme Custom",
      )
    })
    test("has predefined experiences points except custom", () => {
      expect(difficulties->Array.map(experiencePointsForPredefinedDifficulty)) |> toEqual([
        Some(40),
        Some(60),
        Some(80),
        Some(120),
  ...
  describe("difficulty can be converted from and to string", () => {
    difficulties->Array.forEach(difficulty => {
      test("for " ++ difficultyToString(difficulty), () => {
        expect(difficultyFromString(difficultyToString(difficulty))) |> toEqual(difficulty)
      })
    })
  })
})